...
describe('Swap', () => {
    ...
    describe('swapTokenAToB', () => {
        ...
        it('should swap token A to tokenB', async () => {
            ...
            let {deployedSwap, tokenA, tokenB, owner} = await loadFixture(deployContracts);

            await tokenB.transfer(deployedSwap.target, hre.ethers.parseUnits('100000', 18));
            ...
            let balOfContractBeforeSwapInTokenA = await tokenA.balanceOf(deployedSwap.target);
            let balOfContractBeforeSwapInTokenB = await tokenB.balanceOf(deployedSwap.target);
            ...
            await tokenA.approve(deployedSwap.target, hre.ethers.parseUnits('100000', 18));

            console.log('-------------------------Swapping---------------------------------')
            await deployedSwap.swapTokenAToTokenB(hre.ethers.parseUnits("100000", 18));

            let balOfOwnerAfterSwapInTokenA = await tokenA.balanceOf(owner);
            let balOfOwnerAfterSwapInTokenB = await tokenB.balanceOf(owner);

            let balOfContractAfterSwapInTokenA = await tokenA.balanceOf(deployedSwap.target);
            let balOfContractAfterSwapInTokenB = await tokenB.balanceOf(deployedSwap.target);
