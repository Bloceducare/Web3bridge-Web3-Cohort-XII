  ...
  describe("EventContract", () => {
      ...
      describe("Create Event", () => {
          it("should create event", async () => {
              ...
              //console.log(_eventInstance[Symbol])
      ...
      describe("Withdrawal", () => {
        it('Should withdraw event balance', async () => {
          const { _event, owner, address1 } = await loadFixture(deployEventContractFixture);
          const latestTime = await time.latest();
          await _event.createEvent("pool party", "Matured minds only", latestTime+30, latestTime + 86400, 1, 1, 20);
          await _event.connect(address1).registerForEvent(1, {value: 1});
          await time.increase(87000);
          await _event.withdrawForEvent(1);
          expect(await hre.ethers.provider.getBalance(_event.target)).to.equal(0);
        });
        ...
        it('Should revert if non-organizer tries to withdraw', async () => {
          ...
          await _event.connect(address1).registerForEvent(1, {value: 1});
          await time.increase(87000);
          await expect(_event.connect(address1).withdrawForEvent(1))
            .to.be.revertedWith('ONLY ORGANIZER CAN WITHDRAW');
        });
      
        it('Should update event balance to zero after withdrawal', async () => {
          const { _event, owner, address1 } = await loadFixture(deployEventContractFixture);
          const latestTime = await time.latest();
          await _event.createEvent("pool party", "Matured minds only", latestTime+30, latestTime + 86400, 1, 1, 20);
          await _event.connect(address1).registerForEvent(1, {value: 1});
          ...
          expect(await _event.eventBalance(1)).to.equal(0);
        });
      
        it('Should transfer correct amount to organizer', async () => {
          const { _event, owner, address1 } = await loadFixture(deployEventContractFixture);
          const latestTime = await time.latest();
          await _event.createEvent("pool party", "Matured minds only", latestTime+30, latestTime + 86400, 1, 1, 20);
          await _event.connect(address1).registerForEvent(1, {value: 1});
          ...
          expect(finalBalance).to.equal(initialBalance + BigInt(1) - gasUsed);
        });
      });
  });