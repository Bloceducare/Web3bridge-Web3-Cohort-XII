...
const main = async () => {
    ...
    console.log('impersonneted acct eth bal before swap:', ethers.formatUnits(ethBal, 6))

    console.log('impersonneted acct dai bal before swap:', ethers.formatUnits(daiBal, 18))

    let outAmt = ethers.parseUnits('50000', 18);
    //let maxAmt = ethers.parseUnits('60000', 6);
    let deadline = await helpers.time.latest() + 1500;

    console.log('--------------- Approving swap amt ---------------')

    //await usdcContract.connect(impersonatedSigner).approve(UNIRouter, maxAmt);
    ...
    console.log('impersonneted acct usdc bal AF:', ethers.formatUnits(ethBalAfter, 6))
