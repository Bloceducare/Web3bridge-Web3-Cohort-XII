...
contract Swap {
    ...
    event Swapped(address indexed _tokenX, address indexed _tokenY, uint256 amountX, uint256 amountY);
    ...
    function swap(uint256 amountX, uint256 amountY, address to) public {
        ...
        if (to == tokenX || to == tokenY || to == address(0)) revert invalidAddress();
        if (IERC20(tokenX).balanceOf(to) < amountX) revert insufficientBalance();
        if (IERC20(tokenY).balanceOf(to) < amountY) revert insufficientBalance();
        if (amountX != 0 && IERC20(tokenY).balanceOf(address(this)) < 1) revert noLiquidity();
        if (amountY != 0 && IERC20(tokenX).balanceOf(address(this)) < 1) revert noLiquidity();
