...
const main = async () => {
    ...
    const exactDAIOut = ethers.parseUnits('45000', 18);
    const maxUSDCIn = ethers.parseUnits('50000', 6); 
    
    let deadline = await helpers.time.latest() + 1500;

    console.log('--------------- Approving swap amt ---------------')

    await usdcContract.connect(impersonatedSigner).approve(UNIRouter, maxUSDCIn);
