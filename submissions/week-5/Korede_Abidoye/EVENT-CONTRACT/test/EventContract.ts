...
describe("EventContract", () => {
    ...
    describe("Create Event", () => {
        it("should create event", async () => {
            ...
            await _event.createEvent("School Reunion", "Class of '13 Only ", latestTime+30, latestTime + 86400, 1, 1, 20);
            ...
            expect(_eventInstance._title).to.equal("School Reunion");
        });

        it("should not create event if end time is less than start time", async () => {
          const { _event, owner } = await loadFixture(deployEventContractFixture);
          const latestTime = await time.latest();
          await expect(_event.createEvent("School Reunion", "Class of '13 Only ", latestTime+30, latestTime + 10, 1, 1, 20)).to.be.revertedWith('ENDDATE MUST BE GREATER');
        });
        
        it("should not create event if start time is less than current time", async () => {
          const { _event, owner } = await loadFixture(deployEventContractFixture);
          const latestTime = await time.latest();
          await expect(_event.createEvent("School Reunion", "Class of '13 Only ", latestTime-30, latestTime + 10, 1, 1, 20)).to.be.revertedWith('START DATE MUST BE IN FUTURE');
        });

        it("should not create event if event is paid and fee is equal to 0", async () => {
          const { _event, owner } = await loadFixture(deployEventContractFixture);
          const latestTime = await time.latest();
          await expect(_event.createEvent("School Reunion", "Class of '13 Only ", latestTime+30, latestTime + 86400, 0, 1, 20)).to.be.revertedWith("Fee Required For a Paid Event");
        });

        it("should not create if event is free and fee is greater than 0", async () => {
          const { _event, owner } = await loadFixture(deployEventContractFixture);
          const latestTime = await time.latest();
          await expect(_event.createEvent("School Reunion", "Class of '13 Only ", latestTime+30, latestTime + 86400, 1, 0, 20)).to.be.revertedWith("NO Fee Required!!");
        });
        ...
        it("should update event mapping", async () => {
          ...
          const _event_count = await _event.event_count();
          const _eventInstance = await _event.events(_event_count);
          await _event.createEvent("House Party", "Class of '13 Only  PG13", latestTime+30, latestTime + 86400, 1, 1, 20);
        ...
        it("should create event ticket", async () => {
          ...
          expect(await _eventTicket.name()).to.equal("School Reunion");
        });

        it('should emit EventCreated event', async () => {
          const { _event, owner } = await loadFixture(deployEventContractFixture);
          const latestTime = await time.latest();
          await expect(_event.createEvent("School Reunion", "Class of '13 Only ", latestTime+30, latestTime + 86400, 1, 1, 20))
    ...
    describe("Register Event", () => {
      ...
      it("should not register for event if event has ended", async () => {
          const { _event, owner, address1 } = await loadFixture(deployEventContractFixture);
          const latestTime = await time.latest();
          await _event.createEvent("School Reunion", "Class of '13 Only ", latestTime+30, latestTime + 150, 0, 0, 20);
      ...
      it("should not register for event if event is full", async () => {
          const { _event, owner, address1 } = await loadFixture(deployEventContractFixture);
          const latestTime = await time.latest();
          await _event.createEvent("School Reunion", "Class of '13 Only ", latestTime+30, latestTime + 86400, 1, 1, 1);
          await _event.connect(address1).registerForEvent(1, {value: 1});
