...
export const multicall = async (calls) => {
  try {
    ...
    const callData = calls.map(
      ({ address, abi, functionName, params = [] }) => {
    ...
    // Execute the multicall with staticCall (view only)
...
export const fetchPairData = async (pairAddress) => {
  try {
    ...
    // Update to use ethers v6 formatting
    const reserve0Adjusted = ethers.formatUnits(reserves[0], token0Decimals);
    const reserve1Adjusted = ethers.formatUnits(reserves[1], token1Decimals);
