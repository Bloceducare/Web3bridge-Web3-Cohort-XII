...
describe("PiggyBank Contract", function () {
    async function deployPiggyBankFixture() {
        ...
        const PiggyBank = await hre.ethers.getContractFactory("GroupBank");
    ...
    describe("Saving Contributions", function () {
        it("Should allow users to save money", async function () {
            ...
            const saveAmount = hre.ethers.parseEther("2");
        ...
        it("Should increase contributor count when a new user contributes", async function () {
            ...
            await piggyBank.connect(user1).save({ value: hre.ethers.parseEther("1") });
            await piggyBank.connect(user2).save({ value: hre.ethers.parseEther("1") });
        ...
        it("Should not allow zero contribution", async function () {
            ...
            expect(piggyBank.connect(user1).save({ value: 0 })).to.be.revertedWith("YOU ARE BROKE");
        });
    });
    ...
    describe("Withdrawals", function () {
        it("Should allow the manager to withdraw after the withdrawal date", async function () {
            ...
            expect(await hre.ethers.provider.getBalance(piggyBank.getAddress())).to.equal(0);
            expect(await hre.ethers.provider.getBalance(manager.address)).to.be.above(hre.ethers.parseEther("0")); 
        });
        ...
        it("Should not allow withdrawal before the withdrawal date", async function () {
            ...
            expect(piggyBank.withdrawal()).to.be.revertedWith("NOT YET TIME");
        });
        ...
        it("Should not allow withdrawal if target amount is not reached", async function () {
            ...
            expect(piggyBank.withdrawal()).to.be.revertedWith("TARGET AMOUNT NOT REACHED");
        });
        ...
        it("Should only allow the manager to withdraw", async function () {
            ...
            expect(piggyBank.connect(user1).withdrawal()).to.be.reverted;
        });
    });
});